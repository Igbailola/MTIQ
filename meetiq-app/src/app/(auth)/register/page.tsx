      ...
      const { error } = await supabase.auth.signUp({
        ...
        options: {
          emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(nextParam)}${
            nextParam.includes('accept_invite=')
              ? `&accept_invite=${nextParam.match(/[?&]accept_invite=([^&]+)/)?.[1] || ''}`
              : ''
          }`,
        },
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-center text-xs text-muted-foreground mt-4">
        ...
        <LinkComponent href={`/login?next=${encodeURIComponent(nextParam)}`} className="text-accent hover:underline font-semibold">
          Sign in
        </LinkComponent>
      </div>
    </div>
